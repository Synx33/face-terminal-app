...
const cardExitReaderInput = document.getElementById('cardExitReaderInput');
...
async function loadSettings() {
  ...
  cardExitReaderInput.value = s.cardExitReaderNo ?? '';
...
document.getElementById('saveAppSettingsBtn').addEventListener('click', async () => {
  ...
  const cardExitReaderRaw = cardExitReaderInput.value.trim();
  ...
  if (cardExitReaderRaw && (!Number.isInteger(Number(cardExitReaderRaw)) || Number(cardExitReaderRaw) < 1)) {
    appSettingsMsg.className = 'enroll-msg err';
    appSettingsMsg.textContent = 'გასასვლელი წამკითხველის ნომერი უნდა იყოს დადებითი მთელი რიცხვი, ან ცარიელი';
    return;
  }
  ...
  try {
    const res = await fetch('/api/settings/app', {
      ...
      body: JSON.stringify({
        ...
        pollIntervalMs,
        checkoutAfter,
        cardExitReaderNo: cardExitReaderRaw,
      }),
    });
    const result = await res.json();
    if (!res.ok) throw new Error(result.error || 'ვერ შესრულდა');
